...
export default function CompareClient() {
  ...
  // Trend graph viewport gating
  const trendRef = useRef<HTMLDivElement | null>(null)
  const [trendInView, setTrendInView] = useState(false)
  useEffect(() => {
    if (typeof window === "undefined" || !trendRef.current) return
  ...
  // Quiz viewport gating
  const quizRef = useRef<HTMLDivElement | null>(null)
  const [quizInView, setQuizInView] = useState(false)
  useEffect(() => {
    if (typeof window === "undefined" || !quizRef.current) return
      ...
      <section className="py-8 px-4 sm:px-6 lg:px-8">
        <div className="max-w-6xl mx-auto space-y-6">
          ...
          <Card className="print:hidden" style={{ contentVisibility: "auto", containIntrinsicSize: "520px" } as CSSProperties}>
            ...
            <CardContent className="space-y-4" ref={quizRef}>
              {deferNonCritical || !quizInView ? (
              ...
              <div ref={trendRef}>
                {loading || !trendInView ? (
                  ...
                  <TrendGraph
                    ...
                    visible={visible}
                    loading={loading}
                    onToggleVisible={(k) => setVisible((v) => ({ ...v, [k]: !v[k] }))}
                  />
                )}
              </div>
            </CardContent>
