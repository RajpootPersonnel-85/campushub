...
type Mode = "standalone" | "content"

export default function ExamsMegaMenu({ mode = "standalone" }: { mode?: Mode }) {
  const [active, setActive] = useState<(typeof CATEGORIES)[number]>(CATEGORIES[0])
  const items = useMemo(() => EXAMS[active] || [], [active])
  const [dueSoon, setDueSoon] = useState<Array<{ name: string; date: string; days: number }>>([])
  useEffect(() => {
    const today = new Date()
    const computed = UPCOMING_EXAMS
      .map((e) => ({
        ...e,
  ...
  return (
    <div className="relative group">
      ...
      {/* Panel */}
      <div className="invisible opacity-0 group-hover:visible group-hover:opacity-100 transition-[opacity,visibility] duration-150">
        <div className="absolute left-0 top-full mt-3 w-[min(1100px,calc(100vw-2rem))] bg-popover text-popover-foreground border rounded-xl shadow-2xl overflow-hidden">
          {content}
        </div>
      </div>
    </div>
